...
1. **Generation** — rollout generates actions while env steps the sim. The actor
   ...
   the actor's sit idle (on sync mode actor's GPU set usually reassigned to rollout-env in this stage).
...
### 3. Run Modes and critical path

There are two kinds of run modes in embodied RLinf *sync Mode* and *async Mode*. Run Modes are determined by specific RLinf Runners used, `run_embodiment.sh` associate to sync runners, `run_async.sh` associate to async runners.
*Sync Mode* blocks actor when rollout is executed, and trains actor after a full set of trajectories is produced, and then updates rollout model according to actor. Therefore rollout-env and actor typically take all GPUs on seperate time via temporal scheduling

when `env.train.pipeline_stage` is set to `2`, rollout-env will form a pipeline and:
wall time ≈ R*max(T_env, T_rol) + T_act

when `env.train.pipeline_stage` is set to `1`, rollout-env will form not form complete pipeline and:
wall time ≈ R*(T_env + T_rol) + T_act

In this case only different `rollout.predict` time and `env.interact` time will cause bubble(Pattern I Imbalanced Intra Generation Pipeline). Different R*max(T_env, T_rol) and T_act will not cause bubble as they both sit on the critical path(Pattern A — Generation-bound pipeline do not exist in sync mode).

*Async Mode* allows rollout-actor pipeline, rollout and actor workers can be executed simultaneously. However, in async mode rollout model produce trajectories based on earlier version of the model, and actor updates the rollout model routinely, therefore the trajectories used to train the actor may not be produced using the latest actor model, thus overall accuracy may degraded and flucuate more heavliy during the rise(Pattern A — Generation-bound pipeline).

wall time ≈ pipeline_warmup+max(R*max(T_env, T_rol), T_act)

In this case both different `rollout.predict` time and `env.interact` time will cause bubble(Pattern I Imbalanced Intra Generation Pipeline). Different R*max(T_env, T_rol) and T_act will also cause bubble as they both sit on the critical path.

In practice, setting `pipeline_stage` to `1` often couples with `env:0-7` and `rollout: 0-7`, env and rollout use all GPUs at differen times at `pipeline_stage=1`. Setting `env:0-7` and `rollout: 0-7` under such circumstances could have a gain from more rollout/env gpu resources.

*Do not* change original user run mode setting throughout the search.
---

## 5. Memory cosumption model and component offload
...
Golden rule:
Model component memory consumption and offload/onload time cost on your own throughout the searches and emit results in `REPORT.md`
...
- `gpu_utilization.per_process.actor/rollout/env.max_gpu_mem_gib`denote the component memory consumption.
- `offload_cost.actor/rollout/env.offload.mean` and `offload_cost.actor/rollout/env.onload.mean` in `diagnose.json` denote the time cost for onload and offloading the component.

- per-rank GPU peak ≈ sum(resident (non-offloaded) components)+max(the peak of the active(offloaded) components)
e.g. rollout.enable_offload = false and actor.enable_offload = false -> both not offload
placement: env: 0-3 rollout: 4-7 actor: 0-7 -> rollout and actor shares 4-7 but uses in different time and stage.
if your profile result shows rollout.max_gpu_mem_gib≈25 GiB and actor.max_gpu_mem_gib≈61 GiB 
by not offloading rollout and actor their sum could add up to 86GB which could cause OOM in A800

- Fixing everything else, actor peak memory is linear in `micro_batch_size`.env memory consumption grows in a linear fashion(`memory=a*tne+b`) with total num of envs. actor memory consumption grows in a linear fashion(`memory=a*tne+b`) with micro batch size.
...
- `diagnose.py` reports `memory.max_used_pct` and an `oom_risk` band (safe <5GB left at peak ≤ elevated <1GB left at peak ≤ high).
...
With offload on, raising `total_num_envs` or replicas does **not**
raise single-GPU peak memory ( when env is offloaded; replicas add GPUs, not memory).
...
## 8. Missing `metrics.log` = probable OOM/crashed
...
`env_interact_step`, `actor_forward`, …) are warmup (could contain env onload, JIT, cache fill, cudnn
autotune) and are dropped before averaging.
